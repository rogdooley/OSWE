...
class FileTransferServer:
    ...
    def _configure_routes(self):
        ...
        def shutdown():
            if request.remote_addr != "127.0.0.1":
                self.logger.warning(
                    "Unauthorized shutdown attempt from %s", request.remote_addr
                )
                abort(403)
                ...
                if self.encoded:
                    ...
                    if json_response:
                        ...
                        if json_filename and json_encoded_data:
                            save_path = self._decode_and_save(
                                json_encoded_data, json_filename
                            )
                    else:
                    ...
                    self.logger.warning(
                        "No valid upload method detected (encoded or file)"
                    )
                    abort(400)

                self.transfer_count += 1
                self.logger.info(
                    "Saved upload %s (count: %d/%d)",
                    save_path,
                    self.transfer_count,
                    self.limit,
                )
    ...
    def _configure_html_page(self) -> None:
        ...
        def render_transfer_page():
            ...
            return render_template_string(
                html_template, route=self.route, filename=Path(self.file_path).name
            )
    ...
    def start(self):
        self.logger.info(
            "Starting server on port %d with route %s", self.port, self.route
        )
    ...
    def __repr__(self) -> str:
        return (
            f"<FileTransferServer(direction={self.direction}, "
